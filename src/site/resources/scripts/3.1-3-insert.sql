----------------------------------------------------------------
-- RECETTE
----------------------------------------------------------------
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('AD mini');
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('AD moyen');
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('AD maxi');
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('Avancement automatique');
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('Promotion');
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('Promotion au choix');
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('Revalorisation');
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('Titularisation');
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('Int̩gration');
insert into sirh2.p_motif_carriere (lib_motif_carriere) values ('Nomination au choix');
----------------------------------------------------------------
-- PROD
----------------------------------------------------------------
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('AD mini');
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('AD moyen');
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('AD maxi');
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('Avancement automatique');
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('Promotion');
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('Promotion au choix');
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('Revalorisation');
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('Titularisation');
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('Int̩gration');
insert into sirh.p_motif_carriere (lib_motif_carriere) values ('Nomination au choix');
