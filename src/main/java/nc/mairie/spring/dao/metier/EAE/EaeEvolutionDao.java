...
public class EaeEvolutionDao extends EaeDao implements EaeEvolutionDaoInterface {
	...
	public EaeEvolutionDao(EaeDao eaeDao) {
		super.dataSource = eaeDao.getDataSource();
		super.jdbcTemplate = eaeDao.getJdbcTemplate();
		super.NOM_TABLE = "EAE_EVOLUTION";
		super.CHAMP_ID = "ID_EAE_EVOLUTION";
	}
	...
	public EaeEvolution chercherEaeEvolution(Integer idEAE) throws Exception {
		...
		EaeEvolution evol = (EaeEvolution) jdbcTemplate.queryForObject(sql, new Object[] { idEAE },
				new BeanPropertyRowMapper<EaeEvolution>(EaeEvolution.class));

		return evol;
	}

	@Override
	public void modifierMobiliteEaeEvolution(Integer idEaeEvolution, boolean mobGeo, boolean mobFonct, boolean mobServ,
			boolean mobDir, boolean mobColl, boolean mobAutre) throws Exception {
		String sql = "UPDATE " + NOM_TABLE + " set " + CHAMP_MOBILITE_GEO + " =?," + CHAMP_MOBILITE_FONCT + "=?,"
				+ CHAMP_MOBILITE_SERVICE + "=?," + CHAMP_MOBILITE_DIRECTION + "=?," + CHAMP_MOBILITE_COLLECTIVITE
				+ "=?," + CHAMP_MOBILITE_AUTRE + "=? where " + CHAMP_ID + "=?";
	...
	public void modifierChangementMetierEaeEvolution(Integer idEaeEvolution, boolean chgtMetier) throws Exception {
		String sql = "UPDATE " + NOM_TABLE + " set " + CHAMP_CHANGEMENT_METIER + " =? where " + CHAMP_ID + "=?";
	...
	public void modifierDelaiEaeEvolution(Integer idEaeEvolution, String delai) throws Exception {
		String sql = "UPDATE " + NOM_TABLE + " set " + CHAMP_DELAI_ENVISAGE + " =? where " + CHAMP_ID + "=?";
		jdbcTemplate.update(sql, new Object[] { delai, idEaeEvolution });
	}

	@Override
	public void modifierAutresInfosEaeEvolution(Integer idEaeEvolution, boolean concours, boolean vae,
			boolean tempsPartiel, boolean retraite, boolean autrePerspective) throws Exception {
		String sql = "UPDATE " + NOM_TABLE + " set " + CHAMP_CONCOURS + " =?," + CHAMP_VAE + "=?,"
				+ CHAMP_TEMPS_PARTIEL + "=?," + CHAMP_RETRAITE + "=?," + CHAMP_AUTRE_PERSPECTIVE + "=? where "
				+ CHAMP_ID + "=?";
		jdbcTemplate.update(sql,
				new Object[] { concours, vae, tempsPartiel, retraite, autrePerspective, idEaeEvolution });
	}

	@Override
	public void modifierLibelleEaeEvolution(Integer idEaeEvolution, String nomCollectivite, String nomConcours,
			String nomVae, String libAutrePerspective) throws Exception {
		String sql = "UPDATE " + NOM_TABLE + " set " + CHAMP_NOM_COLLECTIVITE + " =?," + CHAMP_NOM_CONCOURS + "=?,"
				+ CHAMP_NOM_VAE + "=?," + CHAMP_LIB_AUTRE_PERSPECTIVE + "=? where " + CHAMP_ID + "=?";
		jdbcTemplate.update(sql, new Object[] { nomCollectivite, nomConcours, nomVae, libAutrePerspective,
				idEaeEvolution });
	}

	@Override
	public void modifierDateRetraiteEaeEvolution(Integer idEaeEvolution, Date dateRetraite) throws Exception {
		String sql = "UPDATE " + NOM_TABLE + " set " + CHAMP_DATE_RETRAITE + " =? where " + CHAMP_ID + "=?";
	...
	public void modifierCommentaireEaeEvaluation(Integer idEaeEvolution, Integer idCree) throws Exception {
		String sql = "UPDATE " + NOM_TABLE + " set " + CHAMP_ID_EAE_COM_EVOLUTION + " =? where " + CHAMP_ID + "=?";
		jdbcTemplate.update(sql, new Object[] { idCree, idEaeEvolution });
	}

	@Override
	public void modifierPourcTpsPartielEaeEvolution(Integer idEaeEvolution, Integer idSpbhorTpsPartiel)
			throws Exception {
		String sql = "UPDATE " + NOM_TABLE + " set " + CHAMP_TEMPS_PARTIEL_ID_SPBHOR + " =? where " + CHAMP_ID + "=?";
	...
	public void creerEaeEvolution(EaeEvolution evolution) throws Exception {
		String sql = "INSERT INTO " + NOM_TABLE + " (" + CHAMP_ID_EAE + ") " + "VALUES (?)";
