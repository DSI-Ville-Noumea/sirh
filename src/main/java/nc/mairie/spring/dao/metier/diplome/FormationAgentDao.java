...
public class FormationAgentDao implements FormationAgentDaoInterface {
	...
	private static Logger logger = LoggerFactory.getLogger(FormationAgentDao.class);
	...
	@Override
	public Integer creerFormationAgent(Integer idTitreFormation, Integer idCentreFormation, Integer idAgent, Integer dureeFormation,
			String uniteDuree, Integer anneeFormation) throws Exception {
		String sql = "INSERT INTO " + NOM_TABLE + " (" + CHAMP_ID_TITRE_FORMATION + "," + CHAMP_ID_CENTRE_FORMATION + "," + CHAMP_ID_AGENT + ","
				+ CHAMP_DUREE_FORMATION + "," + CHAMP_UNITE_DUREE + "," + CHAMP_ANNEE_FORMATION + ") " + "VALUES (?,?,?,?,?,?)";

		jdbcTemplate.update(sql, new Object[] { idTitreFormation, idCentreFormation, idAgent, dureeFormation, uniteDuree, anneeFormation });

		String sqlId = "select * from " + NOM_TABLE + " where " + CHAMP_ID_TITRE_FORMATION + "=? and " + CHAMP_ID_CENTRE_FORMATION + "=? and "
				+ CHAMP_ID_AGENT + "=? and " + CHAMP_DUREE_FORMATION + "=? and " + CHAMP_UNITE_DUREE + "=? and " + CHAMP_ANNEE_FORMATION + "=?";

		FormationAgent form = (FormationAgent) jdbcTemplate.queryForObject(sqlId, new Object[] { idTitreFormation, idCentreFormation, idAgent,
				dureeFormation, uniteDuree, anneeFormation }, new FormationAgentRowMapper());
	...
	public void modifierFormationAgent(Integer idFormation, Integer idTitreFormation, Integer idCentreFormation, Integer idAgent,
		...
		String sql = "UPDATE " + NOM_TABLE + " set " + CHAMP_ID_TITRE_FORMATION + "=? , " + CHAMP_ID_CENTRE_FORMATION + "=?," + CHAMP_ID_AGENT
				+ "=?," + CHAMP_DUREE_FORMATION + "=?," + CHAMP_UNITE_DUREE + "=?," + CHAMP_ANNEE_FORMATION + "=? where " + CHAMP_ID_FORMATION
				+ " =?";

		jdbcTemplate.update(sql,
				new Object[] { idTitreFormation, idCentreFormation, idAgent, dureeFormation, uniteDuree, anneeFormation, idFormation });
	}
