...
public class ServletPrintDocument extends javax.servlet.http.HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (null == nomFichier || "".equals(nomFichier.trim())) {
		...
		try {
			...
			if (OeAVCTFonctArretes.class.getName().equals(fromPage)) {
				...
				if (null != listeImpressionChangementClasse && !"".equals(listeImpressionChangementClasse)) {
					fileByte = sirhService.downloadArrete(
							listeImpressionChangementClasse.toString().replace("[", "").replace("]", "").replace(" ", ""), true,
							Integer.valueOf(anneeSelect), false);
				}
				if (null != listeImpressionAvancementDiff && !"".equals(listeImpressionAvancementDiff)) {
					fileByte = sirhService.downloadArrete(listeImpressionAvancementDiff.toString().replace("[", "").replace("]", "").replace(" ", ""),
							false, Integer.valueOf(anneeSelect), false);
				}
			}
			...
			if (OeAVCTFonctDetaches.class.getName().equals(fromPage)) {
				...
				if (null != listeImpressionChangementClasse && !"".equals(listeImpressionChangementClasse)) {
					fileByte = sirhService.downloadArrete(
							listeImpressionChangementClasse.toString().replace("[", "").replace("]", "").replace(" ", ""), true,
							Integer.valueOf(anneeSelect), true);
				}
				if (null != listeImpressionAvancementDiff && !"".equals(listeImpressionAvancementDiff)) {
					fileByte = sirhService.downloadArrete(listeImpressionAvancementDiff.toString().replace("[", "").replace("]", "").replace(" ", ""),
							false, Integer.valueOf(anneeSelect), true);
				}
			}
			...
			resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Une erreur est survenue pour afficher l'édition " + nomFichier
		...
		try {
			...
			// 33311 : bug sur IE pour ouverture.doc
			if (nomFichier.contains(".doc") || nomFichier.contains(".docx")) {
