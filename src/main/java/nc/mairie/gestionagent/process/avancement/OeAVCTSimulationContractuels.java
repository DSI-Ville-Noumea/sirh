 ...
 * 
 */
public class OeAVCTSimulationContractuels extends nc.mairie.technique.BasicProcess {
	...
	public boolean performPB_LANCER(HttpServletRequest request) throws Exception {
		...
		// et de l'année
		Avancement.supprimerAvancementTravailAvecCategorie(getTransaction(), EnumCategorieAgent.CONTRACTUEL.getLibLong(), an);
	...
	private boolean performCalculContractuel(String codeService, String annee, AgentNW agent) throws Exception {
		...
		if (agent != null) {
			...
			la = AgentNW.listerAgentEligibleAvct(getTransaction(), annee, listeSousService, EnumCategorieAgent.CONTRACTUEL.getLibLong());
		}
		...
		for (AgentNW a : la) {
			...
			if (Services.compareDates(Services.ajouteAnnee(Services.formateDate(carr.getDateDebut()), 2), "01/01/" + annee) >= 0
				...
				if (getTransaction().isErreur()) {
					...
					avct.setTitreFP(tp.getLibTitrePoste().trim());
					...
					avct.setCodeCadre(gg.getCodCadre().trim());
