 ...
 * 
 */
public class OePOSTEDuplicationFDP extends BasicProcess {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	private List<ActionFdpJob> listeJobDuplication;
	...
	public String getJSP() {
		return "OePOSTEDuplicationFDP.jsp";
	}
	...
	public boolean performPB_AFFICHER(HttpServletRequest request) throws Exception {
		// Liste des job de duplication
		setListeJobDuplication(getActionFdpJobDao().listerActionFdpJobDuplication());
		afficheTableau();
		return true;
	}
	...
	private void afficheTableau() throws Exception {
		...
		for (int i = 0; i < getListeJobDuplication().size(); i++) {
			ActionFdpJob actionActi = getListeJobDuplication().get(i);
			Integer j = actionActi.getIdActionFdpJob();
			Agent ag = getAgentDao().chercherAgent(actionActi.getIdAgent());
			addZone(getNOM_ST_LIB_AGENT(j), ag.getNomAgent() + " " + ag.getPrenomAgent());
			addZone(getNOM_ST_DATE(j), sdf.format(actionActi.getDateSubmission()));
			if (actionActi.getStatut() == null) {
				addZone(getNOM_ST_INFO(j), "En attente de traitement");
			} else {
				if (actionActi.getStatut().length() > 150) {
					addZone(getNOM_ST_INFO(j), actionActi.getStatut().substring(0, 150));
				} else {
					addZone(getNOM_ST_INFO(j), actionActi.getStatut());
				}
			}
		}
	}
	...
	public boolean performPB_AFFICHER_ERREUR(HttpServletRequest request) throws Exception {
		// Liste des job de duplication en erreur
		setListeJobDuplication(getActionFdpJobDao().listerActionFdpJobDuplicationErreur());
		afficheTableau();
		return true;
	}
	...
	public List<ActionFdpJob> getListeJobDuplication() {
		return listeJobDuplication == null ? new ArrayList<ActionFdpJob>() : listeJobDuplication;
	}

	public void setListeJobDuplication(List<ActionFdpJob> listeJobDuplication) {
		this.listeJobDuplication = listeJobDuplication;
	}
