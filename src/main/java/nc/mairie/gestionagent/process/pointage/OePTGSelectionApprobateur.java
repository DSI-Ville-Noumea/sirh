		...
		if (zone.length() == 0) {
			...
			for (PositionAdmAgent pa : listeNomatrActif) {
				...
				try {
					...
					Affectation aff = getAffectationDao().chercherAffectationActiveAvecAgent(ag.getIdAgent());
				} catch (Exception e) {
					continue;
				}
				liste += pa.getNomatr() + ",";
			...
			for (Agent ag : listeAgentWithNom) {
				...
				if (paActive == null || getTransaction().isErreur()) {
					...
					try {
						@SuppressWarnings("unused")
						Affectation aff = getAffectationDao().chercherAffectationActiveAvecAgent(agt.getIdAgent());
					} catch (Exception e) {
						continue;
					}
