...
public class OeAGENTEtatCivil extends BasicProcess {
    ...
    public boolean performPB_VALIDER(HttpServletRequest request) throws Exception {
        ...
        {
            setStatut(STATUT_PROCESS_APPELANT, false, MessageUtils.getMessage("INF001", getAgentCourant().getNoMatricule()) + " " + message + " "
                    + messageDateEmbauche);
        } else // "INF001","Agent @ modifié"
        {
            setStatut(STATUT_MEME_PROCESS, false, MessageUtils.getMessage("INF002", getAgentCourant().getNoMatricule()) + " " + messageDateEmbauche);
        }
    ...
    private boolean alimenterAgent(HttpServletRequest request) throws Exception {
        ...
        if (getPaysNaissanceCourant() != null) {
            ...
            getAgentCourant().setCodeCommuneNaissanceEt(((CommuneEtrangere) getCommNaissanceCourant()).getCodCommuneEtrangere());
        } else {
        ...
        getAgentCourant().setDateDerniereEmbauche(
                getVAL_EF_DATE_DERN_EMB().equals(Const.CHAINE_VIDE) ? getVAL_EF_DATE_PREM_EMB() : getVAL_EF_DATE_DERN_EMB());
        ...
        if (getVAL_RG_VILLE_DOMICILE().equals(getNOM_RB_VILLE_DOMICILE_NOUMEA())) {
            ...
            getAgentCourant().setIdVoie(getVoieQuartierCourant() == null ? null : getVoieQuartierCourant().getCodVoie());
        ...
        getAgentCourant().setCodeComVilleBp(getCommuneBPCourant() != null ? getCommuneBPCourant().getCodCommune() : null);


        /**
         * FIX CLV https://redmine.ville-noumea.nc/issues/2424 *
         */
        // Si nom d'usage pas saisi, le renseigner avec le nom Marital, sinon le nom patro
        // RG_AG_EC_C01
        //if (getVAL_EF_NOM_USAGE().length() == 0) {
        //     String nomUsage = (getVAL_EF_NOM_MARITAL().length() != 0 ? getVAL_EF_NOM_MARITAL() : getVAL_EF_NOM_PATRONYMIQUE());
    ...
    private boolean performControlerSaisie(HttpServletRequest request, boolean aValider) throws Exception {
        ...
        //if (getVAL_LB_CIVILITE_SELECT().equals(EnumCivilite.M.getCode()) && getVAL_EF_NOM_MARITAL().length() > 0) {
        ...
        // RG_AG_EC_C03
        //if (getVAL_LB_CIVILITE_SELECT().equals(EnumCivilite.MME.getCode()) && getVAL_EF_NOM_MARITAL().equals(Const.CHAINE_VIDE)) {
        //	getTransaction().declarerErreur(MessageUtils.getMessage("ERR002", "nom marital"));
        //	result &= false;
        //}
        /**
         * FIN FIX CLV https://redmine.ville-noumea.nc/issues/2424 *
    ...
    private boolean performControlerSaisieContact(HttpServletRequest request) throws Exception {
        ...
        // (jusqu'à 50)
            ...
            if (indiceTypeContact >= 0) {
                ...
                if (!EnumTypeContact.EMAIL.getCode().equals(tc.getCodTypeContact()) && getVAL_EF_LIBELLE_CONTACT().length() > 6) {
                    // "ERR018",
                ...
                if (!EnumTypeContact.EMAIL.getCode().equals(tc.getCodTypeContact()) && !Services.estNumerique(getVAL_EF_LIBELLE_CONTACT())) {
    ...
    private void initialiseListeDeroulante() throws Exception {
        ...
        if (getLB_COLLECTIVITE() == LBVide) {
            ...
            int[] tailles = {40};
            String[] champs = {"libLongCollectivite"};
            setLB_COLLECTIVITE(new FormateListe(tailles, col, champs).getListeFormatee());
    ...
    private void initialiseZonesAgentCourant(HttpServletRequest request) throws Exception {
        ...
        for (int i = 0; i < getListeSituation().size(); i++) {
            if (((SituationFamiliale) getListeSituation().get(i)).getIdSituation().equals(getAgentCourant().getIdSituationFamiliale())) {
                indiceSitu = i;
                break;
            }
        }
        ...
        int indiceCol = 0;
        for (int i = 0; i < getListeCollectivite().size(); i++) {
            if (((Collectivite) getListeCollectivite().get(i)).getIdCollectivite().equals(getAgentCourant().getIdCollectivite())) {
                indiceCol = i;
                break;
            }
        }
        ...
        addZone(getNOM_EF_INTITULE_COMPTE(), getAgentCourant().getIntituleCompte() == null ? Const.CHAINE_VIDE : getAgentCourant()
                .getIntituleCompte());
        ...
        if (getAgentCourant().getIdEtatService() == null) {
            ...
            for (int i = 0; i < getListeTypeServiceMilitaire().size(); i++) {
                if (((EtatServiceMilitaire) getListeTypeServiceMilitaire().get(i)).getIdEtatService().equals(getAgentCourant().getIdEtatService())) {
    ...
    private void initialiseZonesLieuNaissance(HttpServletRequest request) throws Exception {
        ...
        if (getLieuNaissance() == null && getAgentCourant() != null) {
            if (getAgentCourant().getCodePaysNaissanceEt() == null) {
                setCommNaissanceCourant(Commune.chercherCommune(getTransaction(), getAgentCourant().getCodeCommuneNaissanceFr()));
                setLieuNaissance(((Commune) getCommNaissanceCourant()).getLibCommune() + " - " + Const.COMMUNE_FRANCE);
            } else {
                ...
                setCommNaissanceCourant(CommuneEtrangere.chercherCommuneEtrangere(getTransaction(), getPaysNaissanceCourant().getCodPays(),
                        getAgentCourant().getCodeCommuneNaissanceEt()));
                setLieuNaissance(((CommuneEtrangere) getCommNaissanceCourant()).getLibCommuneEtrangere() + " - "
                        + getPaysNaissanceCourant().getLibPays());
            }
        }
    ...
    private void initialiseZonesCommune(HttpServletRequest request) throws Exception {
        ...
        if (getCommuneDomCourant() == null && getAgentCourant() != null && getAgentCourant().getCodeComVilleDom() != null
                && !getAgentCourant().getCodeComVilleDom().equals("0")) {
            setCommuneDomCourant(CommunePostal.chercherCommunePostal(getTransaction(), getAgentCourant().getCodePostalVilleDom(), getAgentCourant()
                    .getCodeComVilleDom()));
        }
        ...
        if (getAgentCourant() != null && getAgentCourant().getCodeComVilleBp() != null && !getAgentCourant().getCodeComVilleBp().equals("0")) {
            setCommuneBPCourant(CommunePostal.chercherCommunePostal(getTransaction(), getAgentCourant().getCodePostalVilleBp(), getAgentCourant()
                    .getCodeComVilleBp()));
            if (getTransaction().isErreur()) {
                return;
            }
    ...
    private void initialiseZonesVoieQuartier(HttpServletRequest request) throws Exception {
        ...
        if (getVoieQuartierCourant() == null && getAgentCourant() != null) {
            if (getAgentCourant().getIdVoie() != null && !getAgentCourant().getIdVoie().equals("0")) {
                setVoieQuartierCourant(VoieQuartier.chercherVoieQuartierAvecCodVoie(getTransaction(), getAgentCourant().getIdVoie()));
                addZone(getNOM_RG_VILLE_DOMICILE(), getNOM_RB_VILLE_DOMICILE_NOUMEA());
     ...
     * (17/03/2011 15:26:00)
     *
     * @param newListeSituation ArrayList
     */
     ...
     * 15:26:00)
     *
     * @param newListeBanqueGuichet ArrayList
     */
     ...
     * 10:38:00)
     *
     * @param voieQuartierCourant nc.mairie.metier.commun.VoieQuartier
     */
     ...
     * (24/03/2011 10:37:00)
     *
     * @param newCommuneDomCourant nc.mairie.metier.commun.Commune
     */
     ...
     * (25/03/2011 08:30:00)
     *
     * @param newListeCommuneDomNoumea ArrayList
     */
     ...
     * (25/03/2011 08:30:00)
     *
     * @param newListeCommuneDomAutre ArrayList
     */
     ...
     * 08:22:00)
     *
     * @param newListeTypeServiceMilitaire ArrayList
     */
     ...
     *
     * @param communeBPCourant CommunePostal
     */
     ...
     *
     * @param modeCreation modeCreation à définir
     */
