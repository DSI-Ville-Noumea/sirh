...
public class AlfrescoCMISService implements IAlfrescoCMISService {
	...
	@Override
	public ReturnMessageDto uploadDocument(Integer idAgentOperateur, Agent agent, nc.mairie.metier.agent.Document document, 
			File file, String codTypeDoc) throws Exception {
		return uploadDocument(idAgentOperateur, agent, document, file, null, codTypeDoc);
	}
	
	@Override
	public ReturnMessageDto uploadDocument(Integer idAgentOperateur, Agent agent, nc.mairie.metier.agent.Document document, 
			File file, Integer annee, String codTypeDoc) throws Exception {
		...
		if(null == pathAlfresco
				|| null == pathAlfresco.getPathAlfresco()) {
	    ...
	    if(TYPE_CAMPAGNE_EAE.equals(typeDocument.getCodTypeDocument())
	    		|| TYPE_ACTION_CAMPAGNE_EAE.equals(typeDocument.getCodTypeDocument())) {
	    	String pathCampagneEAE = CmisUtils.getPathSIRH(idAgent, nomUsage, prenomUsage, pathAlfresco.getPathAlfresco())
	    			+ annee + CmisUtils.SLASH;
	    	try {
	    ...
	    String nomDocument = CmisUtils.getPatternSIRH(codTypeDoc, nomUsage, prenomUsage, idAgent, 
	    		new Date(), new DateTime().getMillisOfSecond(), annee);
