	...
	<%@ include file="BanniereErreur.jsp" %>
		<FORM name="formu" method="POST" class="sigp2-titre">
			...
			<FIELDSET class="sigp2Fieldset"  style="text-align: left; margin: 10px; width:500px; float:left;">			
				...
				<span class="sigp2-titre" align="center" colspan="2">
				<SELECT name="<%= process.getNOM_LB_CADRE() %>" size="10"
					style="width:100%;" class="sigp2-liste">
					<%=process.forComboHTML(process.getVAL_LB_CADRE(), process.getVAL_LB_CADRE_SELECT()) %>
				</SELECT>
            	</span>            	
				<div class=<%=MairieUtils.getNomClasseCSS(request, process.getNomEcran(), EnumTypeDroit.EDITION, "") %>>
					<INPUT tabindex="" type="image" src="images/ajout.gif" height="20px" width="20px" name="<%=process.getNOM_PB_CREER_CADRE()%>">
    	        	<INPUT tabindex="" type="image" src="images/suppression.gif" height="20px" width="20px" name="<%=process.getNOM_PB_SUPPRIMER_CADRE()%>">
    	        </div>
            	<% if (process.getVAL_EF_ACTION_CADRE()!= null && !process.getVAL_EF_ACTION_CADRE().equals("")) {%>
            	<br>
				<% if (!process.ACTION_SUPPRESSION.equals(process.getVAL_EF_ACTION_CADRE())) { %>
					<label class="sigp2Mandatory" Style="width:50px">Libellé:</label>
					<INPUT tabindex="" class="sigp2-saisiemajuscule" maxlength="100" name="<%= process.getNOM_EF_CADRE() %>" size="35" type="text" value="<%= process.getVAL_EF_CADRE() %>" style="margin-right:10px;margin-bottom:10px">
					<br />					
					<div Style="width:100%" align="center">					
					<% if (process.ACTION_CREATION.equals(process.getVAL_EF_ACTION_CADRE())) { %>
						<span class="sigp2"><INPUT type="submit" class="sigp2-Bouton-100" value="Ajouter" name="<%=process.getNOM_PB_VALIDER_CADRE()%>"></span>
					<% } %>
				<%} else {%>
					<label class="sigp2Mandatory" Style="width:50px">Libellé:</label>
					<INPUT readonly="readonly" tabindex="" class="sigp2-saisiemajuscule" maxlength="100" name="<%= process.getNOM_EF_CADRE() %>" size="35" type="text" value="<%= process.getVAL_EF_CADRE() %>" style="margin-right:10px;margin-bottom:10px">
					<br />					
					<div Style="width:100%" align="center">
					<span class="sigp2"><INPUT type="submit" class="sigp2-Bouton-100" value="Supprimer" name="<%=process.getNOM_PB_VALIDER_CADRE()%>"></span>
			   <%}%>
				<span class="sigp2"><INPUT type="submit" class="sigp2-Bouton-100" value="Annuler" name="<%=process.getNOM_PB_ANNULER_CADRE()%>"></span>
				</div>
				<% } %>			
			</FIELDSET>			
			<FIELDSET class="sigp2Fieldset"  style="text-align: left; margin: 10px; width:500px; float:left;">			
