...
<%@page import="nc.mairie.spring.domain.metier.specificites.PrimePointageAff"%>
...
<jsp:useBean
	class="nc.mairie.gestionagent.process.agent.OeAGENTEmploisSpecificites"
	id="process" scope="session"></jsp:useBean>
<BODY bgcolor="#FFFFFF" BGPROPERTIES="FIXED"
	background="images/fond.jpg" lang="FR" link="blue" vlink="purple"
	onload="window.parent.frames('refAgent').location.reload();return setfocus('<%=process.getFocus()%>')">
	...
	<FORM name="formu" method="POST" class="sigp2-titre">
		...
		<FIELDSET class="sigp2Fieldset"
			...
			<legend class="sigp2Legend">Gestion des spécificités d'une
				fiche de poste</legend>
			<br />
			<div align="left">
				<span class="sigp2-RadioBouton"> <INPUT tabindex=""
					type="radio"
					<%=process.forRadioHTML(process.getNOM_RG_SPECIFICITE(), process.getNOM_RB_SPECIFICITE_AN())%>
					onclick='executeBouton("<%=process.getNOM_PB_CHANGER_SPECIFICITE()%>")'>Avantage
					en nature <span style="width: 5px"></span> <INPUT tabindex=""
					type="radio"
					<%=process.forRadioHTML(process.getNOM_RG_SPECIFICITE(), process.getNOM_RB_SPECIFICITE_D())%>
					onclick='executeBouton("<%=process.getNOM_PB_CHANGER_SPECIFICITE()%>")'>Délégation
					<span style="width: 5px"></span> <INPUT tabindex="" type="radio"
					<%=process.forRadioHTML(process.getNOM_RG_SPECIFICITE(), process.getNOM_RB_SPECIFICITE_RI())%>
					onclick='executeBouton("<%=process.getNOM_PB_CHANGER_SPECIFICITE()%>")'>Régime
					indemnitaire <span style="width: 5px"></span> <INPUT tabindex=""
					type="radio"
					<%=process.forRadioHTML(process.getNOM_RG_SPECIFICITE(), process.getNOM_RB_SPECIFICITE_PP())%>
					onclick='executeBouton("<%=process.getNOM_PB_CHANGER_SPECIFICITE()%>")'>Prime
					pointage
				</span> <INPUT type="submit" style="visibility: hidden;"
					name="<%=process.getNOM_PB_CHANGER_SPECIFICITE()%>" value="OK">
			</div>
			<BR />
			<%
				...
				<div align="left" style="float: left; width: 100%; display: block;">
					...
					<div align="left" style="float: left; width: 100%; display: none;">
						...
						<table class="sigp2-tab">
							...
							%>
							<tr>
								<td class="sigp2-tabLigne" align="center"><INPUT
									tabindex="" type="image" src="images/suppression.gif"
									class="<%=MairieUtils.getNomClasseCSS(request, process.getNomEcran(), EnumTypeDroit.EDITION, "")%>"
									height="16px" width="16px"
									...
									<%
											...
											if (process.getListePrimePointageFP() != null) {
												//System.out.println("------\nfp:");
													...
													%>
													<tr>
														<td class="sigp2-tabLigne"><input tabindex="" type="image" src="images/check.gif" name="<%=process.getNomPrimeFP_AFF(indicePrimePointage)%>" value="checked"></td>
													...
													//System.out.println("prime:"+ process.getVAL_ST_LST_PRIME_POINTAGE_RUBRIQUE(indicePrimePointage)+"  index="+indicePrimePointage);
													indicePrimePointage++;
												}
											}
											if (process.getListePrimePointageAFF() != null  && process.getListePrimePointageAFF().size()>0) {
												...
												%>
												<tr>
													<td class="sigp2-tabLigne" align="center"><INPUT tabindex="" type="image" src="images/suppression.gif" class="<%=MairieUtils.getNomClasseCSS(request, process.getNomEcran(), EnumTypeDroit.EDITION, "")%>" height="16px" width="16px" name="<%=process.getNOM_PB_SUPPRIMER_PRIME_POINTAGE(indicePrimePointage)%>"> </td>
													<td class="sigp2-tabLigne"><%=process.getVAL_ST_LST_PRIME_POINTAGE_RUBRIQUE(indicePrimePointage)%></td>
		...
		<FIELDSET class="sigp2Fieldset"
			...
			<legend class="sigp2Legend"><%=process.getVAL_ST_ACTION()%>
			...
			<span class="sigp2Mandatory"
				...
				name="<%=process.getNOM_LB_TYPE_AVANTAGE()%>" style="width: 350px;">
			...
			<span class="sigp2Mandatory"
				...
				: </span> <span> <SELECT class="sigp2-liste"
				name="<%=process.getNOM_LB_TYPE_REGIME()%>" style="width: 350px;">
				...
				: </span> <span> <INPUT class="sigp2-saisie" maxlength="8"
				name="<%=process.getNOM_EF_FORFAIT_REGIME()%>" size="10" type="text"
				value="<%=process.getVAL_EF_FORFAIT_REGIME()%>">
			</span> <BR /> <BR /> <span class="sigp2"
		...
		<FIELDSET
			...
			style="text-align: center; width: 1030px;">
			<INPUT type="submit" class="sigp2-Bouton-100" value="Valider"
				name="<%=process.getNOM_PB_VALIDER()%>"> <INPUT
				type="submit" class="sigp2-Bouton-100" value="Annuler"
