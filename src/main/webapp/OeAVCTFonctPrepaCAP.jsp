				...
				<table class="display" id="tabAvctFonct">
					...
					%>
							<tr>
								...
								<td>								
									<%if(avct.getAvisShd()!=null &&avct.getIdMotifAvct()!=null && String.valueOf(avct.getIdMotifAvct()).equals("4")){%>
									...
									<%}else if( avct.getAvisShd()!=null &&avct.getIdMotifAvct()!=null && String.valueOf(avct.getIdMotifAvct()).equals("7")){ %>
									...
									<%}else if(avct.getAvisShd()!=null &&avct.getIdMotifAvct()!=null && String.valueOf(avct.getIdMotifAvct()).equals("6")){ %>
										<SELECT disabled="disabled"  name="<%= process.getNOM_LB_AVIS_CAP_AD(indiceAvct) %>" class="sigp2-liste" >
												<%=process.forComboHTML(process.getVAL_LB_AVIS_CAP_AD(indiceAvct), process.getVAL_LB_AVIS_CAP_AD_SELECT(indiceAvct)) %>
										</SELECT>
									<%} else{%>&nbsp;
								...
								<td>
									<%if(avct.getAvisShd()!=null && avct.getIdMotifAvct()!=null && null != avct.getIdAvisCap()){%>
										<%if(String.valueOf(avct.getIdMotifAvct()).equals("7") && avct.getIdAvisCap() == 1){%>
								...
								<td>
									<%if(avct.getIdMotifAvct()!=null && (String.valueOf(avct.getIdMotifAvct()).equals("7") || String.valueOf(avct.getIdMotifAvct()).equals("4")|| String.valueOf(avct.getIdMotifAvct()).equals("6")|| String.valueOf(avct.getIdMotifAvct()).equals("3"))){ %>
									<INPUT type="checkbox" onClick='validSEF("<%=indiceAvct %>")'  <%= process.forCheckBoxHTML(process.getNOM_CK_VALID_SEF(indiceAvct),process.getVAL_CK_VALID_SEF(indiceAvct))%>>
								</td>
								<%} %>
				...
				<table class="display" id="tabAvctFonctImpr">
					...
					%>
							<tr>
								...
								<td align="center" ><INPUT title="consulter" type="image" src="images/oeil.gif" height="15px" width="15px" class="<%= MairieUtils.getNomClasseCSS(request, process.getNomEcran(), EnumTypeDroit.EDITION, "") %>" name="<%=process.getNOM_PB_CONSULTER_TABLEAU_AVIS_VDN(indiceImpr,process.getVAL_ST_CODE_CAP(indiceImpr),process.getVAL_ST_CADRE_EMPLOI(indiceImpr))%>"></td>
